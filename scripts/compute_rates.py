...
from proving_ground.hardness import is_confusion_non_degenerate, is_reference_only, is_trivial_tautology
...
def _classify_entry(entry: dict, run_target: str | None) -> str:
    ...
    if "is_degenerate" not in entry:
        ...
        if is_trivial_tautology(decomp):
            return "tautology"
        if is_reference_only(decomp):
            return "reference"
        return "confusion" if is_confusion_non_degenerate(decomp) else "structured"
    if entry["is_degenerate"]:
        return "degenerate"
    # For post-classifier runs: always recompute tautology and reference from subgoal
    # strings, since they weren't written to disk in older run files.
...
def compute_model_rates(runs: list[dict]) -> dict[str, dict]:
    ...
    for model, s in stats.items():
        ...
        non_degen = s["confusion"] + s["tautology"] + s.get("reference", 0) + s["structured"] + s["unknown_nondegen"]
...
def print_rates(problem_id: str, runs: list[dict], stats: dict[str, dict], three_way: bool = False) -> None:
    ...
    print(f"{'='*70}")
    if three_way:
        print(f"{'Model':<30} {'N':>4} {'Deg':>5} {'Conf':>5} {'Taut':>5} {'Ref':>5} {'Struct':>7} {'Unk':>4} {'DegRate':>8} {'95% CI':>14}")
        print(f"{'-'*30} {'-'*4} {'-'*5} {'-'*5} {'-'*5} {'-'*5} {'-'*7} {'-'*4} {'-'*8} {'-'*14}")
        for model, s in sorted(stats.items()):
            ...
            print(
                ...
                f" {s.get('tautology', 0):>5} {s.get('reference', 0):>5} {s['structured']:>7} {s['unknown_nondegen']:>4} {deg_rate:>8} {ci:>14}"
            )
    else:
